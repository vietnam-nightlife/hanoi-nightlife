    ...
    <Link href={`/${place.city}/${place.category}/${place.slug}`}>
      <article className="card-hover overflow-hidden rounded-3xl border border-white/10 bg-[#101010]">
        <div className="relative h-56 overflow-hidden">
          ...
          {place.featured && (
            <span className="absolute left-4 top-4 rounded-md bg-red-600 px-3 py-2 text-xs font-black">
              추천 인기 업소
            </span>
          )}
        </div>
        ...
        <div className="p-6">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2 text-sm">
            ...
            {place.koreanSupport && (
              <span className="inline-flex items-center rounded-md bg-blue-500/20 px-2.5 py-1 text-xs font-bold text-blue-400">
                🔵 한국어 응대 가능
              </span>
            )}

            {place.revisitRecommended && (
              <span className="inline-flex items-center rounded-md bg-red-500/20 px-2.5 py-1 text-xs font-bold text-red-400">
                🔥 재방문 추천
              </span>
            )}
          </div>
